...
static int8_t BME280_I2C_bus_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr);
static void bme280_error_codes_print_result(const char api_name[], int8_t rslt);
static int8_t BME280_I2C_bus_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len,
        void *intf_ptr);

static inline double CtoF(double tempC)
{
	return (tempC * 9 / 5) + 32;
}
...
int sensors_init()
{
	...
	dev.delay_us = BME280_delay_usec;
...
double sensors_temp()
{
	return CtoF(comp_data.temperature);
}
